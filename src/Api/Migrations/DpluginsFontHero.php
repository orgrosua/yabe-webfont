...
class DpluginsFontHero extends AbstractApi implements ApiInterface
{
    ...
    {
        $hooks = [
            'a!yabe/webfont/api/font:migration',
            'a!yabe/webfont/api/font:clean_up',
        ];
    ...
    public function register_custom_endpoints(): void
    {
        register_rest_route(
            self::API_NAMESPACE,
            $this->get_prefix() . '/import-fonts',
            [
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => fn (\WP_REST_Request $wprestRequest): \WP_REST_Response => $this->import_fonts($wprestRequest),
        ...
        register_rest_route(
            self::API_NAMESPACE,
            $this->get_prefix() . '/clean-up',
            [
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => fn (\WP_REST_Request $wprestRequest): \WP_REST_Response => $this->clean_up($wprestRequest),
                'permission_callback' => fn (\WP_REST_Request $wprestRequest): bool => $this->permission_callback($wprestRequest),
            ]
        );
    }
    ...
    public function import_fonts(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        if (! defined('DP_FH_BASE')) {
            return new WP_REST_Response([
                'message' => 'Font Hero plugin is not activated',
            ], 404);
        }
        ...
        foreach ($dpfh_fonts as $dpfh_font) {
            ...
            $dpfh_font_faces = $wpdb->get_results(
                $wpdb->prepare(
                    sprintf('SELECT * FROM %sdp_fh_font_faces WHERE font_id = %%d', $wpdb->prefix),
                    $dpfh_font->id
                )
            );
            ...
            foreach ($dpfh_font_faces as $dpfh_font_face) {
                ...
                $font_face = array_filter(
                    $font_faces,
                    static fn ($ff) => $ff['weight'] === $f_weight && $ff['style'] === $f_style
                );
                ...
                if ($font_face !== []) {
                    ...
                    $font_faces = array_filter(
                        $font_faces,
                        static fn ($ff) => $ff['id'] !== $font_face['id']
                    );
                } else {
                    $font_face = [
                        ...
                        'width' => '',
                        'style' => $f_style,
                        'display' => empty($dpfh_font_face->font_display) ? '' : sanitize_text_field($dpfh_font_face->font_display),
                        'selector' => '',
                        'comment' => '',
                        'unicodeRange' => '',
                        'preload' => $dpfh_font_face->font_preload === 'yes',
                        'files' => [],
                    ];
                }
                ...
                if (! empty($dpfh_font_face->font_file)) {
                    ...
                    $file_name = sanitize_title_with_dashes(sprintf(
                        '%s-%s-%s-%s-%s',
                        sanitize_text_field($dpfh_font->font_name),
                        $f_weight,
                        $f_style,
            ...
            $wpdb->insert(sprintf('%syabe_webfont_fonts', $wpdb->prefix), [
                'type' => 'custom',
                'title' => sanitize_text_field($dpfh_font->font_name),
                'slug' => Common::random_slug(10),
                'family' => sanitize_text_field($dpfh_font->font_name),
                'status' => true,
