...
class AdobeFonts extends AbstractApi implements ApiInterface
{
    ...
    public function register_custom_endpoints(): void
    {
        register_rest_route(
            self::API_NAMESPACE,
            $this->get_prefix() . '/get-kits',
            [
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => fn (\WP_REST_Request $wprestRequest): \WP_REST_Response => $this->get_kits($wprestRequest),
        ...
        register_rest_route(
            self::API_NAMESPACE,
            $this->get_prefix() . '/sync',
            [
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => fn (\WP_REST_Request $wprestRequest): \WP_REST_Response => $this->sync($wprestRequest),
                'permission_callback' => fn (\WP_REST_Request $wprestRequest): bool => $this->permission_callback($wprestRequest),
            ]
        );
    ...
    public function get_kits(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        $kits = json_decode($body, true, 512, JSON_THROW_ON_ERROR);
        ...
        foreach ($kits as $kit) {
            $wpdb->insert(
                ...
                [
                    ...
                    'slug' => $kit['id'],
                    'family' => $kit['slug'],
                    'metadata' => json_encode($kit, JSON_THROW_ON_ERROR),
                    'font_faces' => json_encode([], JSON_THROW_ON_ERROR),
                ],
                [
                    '%d',
                    '%s',
