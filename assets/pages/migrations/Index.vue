<template>
    ...
    <div class="mt-4">
        <div class="divide-y divide-gray-200 overflow-hidden rounded-lg bg-gray-200 shadow sm:grid sm:grid-cols-2 sm:gap-px sm:divide-y-0">
            <div v-for="(legacy, legacyIdx) in legacies" :key="legacy.title" :class="[legacyIdx === 0 ? 'rounded-tl-lg rounded-tr-lg sm:rounded-tr-none' : '', legacyIdx === 1 ? 'sm:rounded-tr-lg' : '', legacyIdx === legacies.length - 2 ? 'sm:rounded-bl-lg' : '', legacyIdx === legacies.length - 1 ? 'rounded-bl-lg rounded-br-lg sm:rounded-bl-none' : '', 'group relative bg-white p-6 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500']">
                <div>
                    <span :class="[legacy.iconBackground, legacy.iconForeground, 'inline-flex rounded-lg p-3 ring-4 ring-white']">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 fill-current" aria-hidden="true" viewBox="0 0 576 512"><!--! Font Awesome Pro 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
                ...
                <div class="mt-8">
                    <h3 class="text-base font-semibold leading-6 text-gray-900">
                        <router-link :to="{ name: legacy.route }" class="no-underline text-black focus:outline-none">
                            <!-- Extend touch target to entire panel -->
                            <span class="absolute inset-0" aria-hidden="true" />
                            {{ legacy.title }}
                        </router-link>
                    </h3>
                    <p class="mt-2 text-sm text-gray-500">{{ legacy.description }}</p>
                </div>
